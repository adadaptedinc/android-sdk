...
class ConfigTest {
    ...
    private val eventServerVersion = "/v/0.9.5/"
    ...
    fun serverUrlsUseSandboxByDefault() {
        ...
        assertEquals(ServerUrl.SAND_AD_SERVER.plus(eventServerVersion).plus("android/ads/events"), Config.getAdEventsUrl())
        assertEquals(ServerUrl.SAND_AD_SERVER.plus(adServerVersion).plus("intercept/retrieve"), Config.getRetrieveInterceptsUrl())
        assertEquals(ServerUrl.SAND_AD_SERVER.plus(eventServerVersion).plus("android/intercepts/events"), Config.getInterceptEventsUrl())
    ...
    fun settingProdUsesProdUrls() {
        ...
        assertEquals(ServerUrl.PROD_AD_SERVER.plus(eventServerVersion).plus("android/ads/events"), Config.getAdEventsUrl())
        assertEquals(ServerUrl.PROD_AD_SERVER.plus(adServerVersion).plus("intercept/retrieve"), Config.getRetrieveInterceptsUrl())
        assertEquals(ServerUrl.PROD_AD_SERVER.plus(eventServerVersion).plus("android/intercepts/events"), Config.getInterceptEventsUrl())
