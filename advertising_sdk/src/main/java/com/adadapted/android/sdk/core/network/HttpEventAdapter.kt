...
class HttpEventAdapter(private val adEventUrl: String, private val sdkEventUrl: String, private val errorUrl: String, private val httpConnector: HttpConnector) :
    EventAdapter {
    override suspend fun publishAdEvents(sessionId: String, deviceInfo: DeviceInfo, adEvents: List<AdEvent>) {
        try {
            httpConnector.client.post(adEventUrl) {
                contentType(ContentType.Application.Json)
                setBody(EventRequestBuilder.buildAdEventRequest(sessionId, deviceInfo, adEvents))
    ...
    override suspend fun publishSdkEvents(sessionId: String, deviceInfo: DeviceInfo, events: Set<SdkEvent>) {
        try {
            httpConnector.client.post(sdkEventUrl) {
                contentType(ContentType.Application.Json)
                setBody(EventRequestBuilder.buildEventRequest(sessionId, deviceInfo, sdkEvents = events))
                header(API_HEADER, deviceInfo.appId)
            }
        } catch (e: Exception) {
            e.message?.let { AALogger.logError(it) }
    ...
    override suspend fun publishSdkErrors(sessionId: String, deviceInfo: DeviceInfo, errors: Set<SdkError>) {
        try {
            httpConnector.client.post(errorUrl) {
                contentType(ContentType.Application.Json)
                setBody(EventRequestBuilder.buildEventRequest(sessionId, deviceInfo, sdkErrors = errors))
                header(API_HEADER, deviceInfo.appId)
            }
        } catch (e: Exception) {
